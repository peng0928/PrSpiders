...
class PrSpiders(settions):
    ...
    def fetch(
        ...
        if response:
            if response.ok:
                ...
                loguer.info(f"{method} | {response.code} | {url}")
                return callable(callback(response))
            else:
                settions.false_num += 1
                loguer.error(
                    f"{method} {response.code} {url}"
                )
                return callable(callback(response))
        else:
            settions.false_num += 1
            loguer.error(f"{method} {response.code} {url}")
            callback(response)
            return self
    ...
    def loggering(self, file_log, level="info"):
        ...
        stdout_handler = {
            ...
            "format": "<light-green><b>{time:YYYY-MM-DD HH:mm:ss.SSS}</b></light-green> | <level>{level: ^8}</level> | {message}"
        }
